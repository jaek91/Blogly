...
@app.route('/')
def init():
    """displays top 5 posts and user info"""
...
def display_user_posts():
    """Shows list of top 5 most recent posts in db"""
...
def new_users():
    ...
    if (request.method == "POST"):
        ...
        db.session.commit()
        flash(f"New User {new_user.full_name} successfully created!")
...
def edit_user(id):
    user = User.query.get_or_404(id)
    if (request.method =="GET"):
        return render_template('users/user_edit_form.html', user=user)
...
def new_post(id):
    ...
    tags = Tag.query.all()
    if (request.method =="GET"):
        return render_template("posts/posts_show.html", user = user, tags = tags)
...
def edit_post(id):
    ...
    if (request.method =="GET"):
        return render_template("posts/post_edit.html", post = post, tags = tags)
...
def get_tag(id):
    ...
    return render_template("tags/tags_details.html", tag = tag)
...
def add_new_tags():
    ...
    if (request.method =="GET"):
        return render_template("tags/tags_form.html", posts = posts)
...
def edit_tags(id):
    ...
    if (request.method == "GET"):
        return render_template("tags/tags_edit.html", tag = tag, posts = posts)
